...
Mooncake Store 优势的地方：
1. 传输层抽象为 Transport：
    - 通过多态的方式支持多种不同的传输协议，易于扩展，Ascend 已经充分证明了这一点
    - 传输层均为异步传输接口，充分利用数据pipeline传输的优势
2. 拓扑感知：
    - 多种传输协议可以同时存在，如 `TCP/RDMA`，拓扑可以自动感知两节点的最优路径，并且在最优路径无法打通时降级为次一级连接，提升可靠性
    - 中心化master管理了所有数据节点的资源，拓扑的逻辑非常简单
3. 极其轻量化的数据节点，数据节点只需负责读写数据，职责非常清晰

Mooncake Store 劣势地方：
1. 单 master 肉眼可见会带来单点瓶颈，当节点个数较多，请求较为频繁时，元数据服务器会比较繁忙。
2. 单 master HA 能力肉眼可见的差，如果启用 ETCD 做主备的话又会面临元数据操作过慢的问题。
3. TE 看起来似乎没做什么可靠性处理，节点挂了并不知道

Mooncake Store 待定设计点：
1. 所有的内存分配/释放均由 master 完成，拓扑好做了但是单master瓶颈又是一个问题了


数据系统有什么可以参考的点：
1. 传输层的抽象：当前也有多种传输协议URMA/RDMA/TCP/HCCS，也是需要抽象层的，甚至应该把mmap的这种机制也做成一种传输层
2. 缓存层的抽象应当不止于mmap这种，不被共享内存机制绑死
3. 拓扑可以有效提升传输过程中的DFX能力，特别是对脆弱的URMA来说